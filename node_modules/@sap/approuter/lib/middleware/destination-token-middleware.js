...
const retrieveDestination = require('./destination-token-handler').retrieveDestination;
const addDestinationHeaders = require('./destination-token-handler').addDestinationHeaders;
const isDestinationServiceFlow = require('./destination-token-handler').isDestinationServiceFlow;
...
function getDestination (req, res, options, next){
  let cachedDestination = getCachedDestination(req,options.destinationName);
  if (cachedDestination){
    updateInternalUrl(req,cachedDestination.destinationConfiguration);
    addDestinationHeaders(req);
    return next();
  }
  return retrieveDestination(options, function (err, result) {
    ...
    sessionExt.update(req.session, function(// eslint-disable-next-line
      ...
      req.session.user.destinations[result.destination.name].expireDate = result.expireDate;
      req.session.user.destinations[result.destination.name].destinationConfiguration = result.destination;
...
function getCachedDestination(req, destinationName){
  let destination = req.session && req.session.user && req.session.user.destinations && req.session.user.destinations[destinationName];
